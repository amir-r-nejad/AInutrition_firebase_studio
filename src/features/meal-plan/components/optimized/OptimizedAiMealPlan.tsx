...
interface OptimizedAiMealPlanProps {
  ...
  onOptimizationComplete?: (updatedPlan: GeneratePersonalizedMealPlanOutput) => void;
}
...
export default function OptimizedAiMealPlan({
  ...
  const [optimizationState, setOptimizationState] = useState<OptimizationState | null>(null);
  ...
  const calculateMacroAccuracy = (actual: number, target: number) => {
    ...
    const accuracy = Math.max(0, 100 - Math.abs((actual - target) / target) * 100);
    return Math.round(accuracy);
  };

  const getMacroStatus = (accuracy: number) => {
    if (accuracy >= 95) return { color: "text-green-600", bg: "bg-green-100", status: "Excellent" };
    if (accuracy >= 90) return { color: "text-blue-600", bg: "bg-blue-100", status: "Good" };
    if (accuracy >= 80) return { color: "text-yellow-600", bg: "bg-yellow-100", status: "Fair" };
    return { color: "text-red-600", bg: "bg-red-100", status: "Needs Optimization" };
  };
  ...
  const sumMealFromIngredients = (meal: any) => {
    ...
    return ingredients.reduce(
      (
        ...
        const cals = Number(ing?.calories ?? (ing as any)?.kcal ?? (ing as any)?.cal ?? (ing as any)?.energy_kcal ?? 0) || 0;
        const prot = Number(ing?.protein ?? (ing as any)?.prot ?? (ing as any)?.proteins ?? (ing as any)?.protein_g ?? 0) || 0;
        const carbs = Number(ing?.carbs ?? (ing as any)?.carb ?? (ing as any)?.carbohydrates ?? (ing as any)?.carbohydrates_g ?? 0) || 0;
        const fat = Number(ing?.fat ?? (ing as any)?.fats ?? (ing as any)?.fat_g ?? 0) || 0;
        return {
  ...
  const optimizeMeal = async (dayIndex: number, mealIndex: number, targetMacros: any) => {
    setOptimizationState({ dayIndex, mealIndex, isOptimizing: true });

    startTransition(async () => {
      try {
        console.log("🔧 Starting meal optimization:", { dayIndex, mealIndex, targetMacros });
        ...
        toast({
          ...
          description: error.message || "Failed to optimize meal. Please try again.",
          variant: "destructive",
        });
      } finally {
            ...
            <CardContent>
              <div className="grid gap-4">
                {day?.meals?.map((meal, mealIndex) => {
                  ...
                  let totalCals = meal.total_calories ?? (meal as any).totalCalories ?? 0;
                  let totalProt = meal.total_protein ?? (meal as any).totalProtein ?? 0;
                  let totalCarbs = meal.total_carbs ?? (meal as any).totalCarbs ?? 0;
                  ...
                  const caloriesAccuracy = mealTargets ? 
                    calculateMacroAccuracy(totalCals, mealTargets.calories) : 100;
                  const proteinAccuracy = mealTargets ? 
                    calculateMacroAccuracy(totalProt, mealTargets.protein) : 100;
                  const carbsAccuracy = mealTargets ? 
                    calculateMacroAccuracy(totalCarbs, mealTargets.carbs) : 100;
                  const fatAccuracy = mealTargets ? 
                    calculateMacroAccuracy(totalFat, mealTargets.fat) : 100;
                  ...
                  const isCurrentlyOptimizing = optimizationState?.dayIndex === dayIndex && 
                    optimizationState?.mealIndex === mealIndex && optimizationState?.isOptimizing;
                    ...
                    <Card key={mealIndex}>
                      <CardHeader className="pb-3">
                        <div className="flex items-center justify-between">
                          <div>
                            <CardTitle className="text-lg">{meal.meal_title}</CardTitle>
                            <p className="text-sm text-gray-600 mt-1">{meal.meal_name}</p>
                          </div>
                          <div className="flex items-center gap-2">
                            ...
                            {mealTargets && overallAccuracy < 95 && (
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => optimizeMeal(dayIndex, mealIndex, mealTargets)}
                                disabled={isOptimizing}
                                className="flex items-center gap-1"
                              >
                                <Zap className="h-3 w-3" />
                                {isCurrentlyOptimizing ? "Optimizing..." : "Optimize"}
                              </Button>
                            )}
                          </div>
                        </div>
                      ...
                      <CardContent>
                          ...
                          <div className="mb-4 space-y-2">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                              <div>
                                ...
                                <Progress value={caloriesAccuracy} className="h-2" />
                                <div className="text-xs">{caloriesAccuracy}%</div>
                              </div>
                              <div>
                                <div className="text-xs text-gray-600">Protein</div>
                                <Progress value={proteinAccuracy} className="h-2" />
                                <div className="text-xs">{proteinAccuracy}%</div>
                              </div>
                              <div>
                                <div className="text-xs text-gray-600">Carbs</div>
                                <Progress value={carbsAccuracy} className="h-2" />
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                          <div className="text-center">
                            ...
                            <div className="text-xs text-gray-600">calories</div>
                            {mealTargets && (
                              <div className="text-xs text-gray-500">/ {mealTargets.calories}</div>
                            )}
                          </div>
                          <div className="text-center">
                            ...
                            <div className="text-xs text-gray-600">protein</div>
                            {mealTargets && (
                              <div className="text-xs text-gray-500">/ {mealTargets.protein}g</div>
                            )}
                          </div>
                          <div className="text-center">
                            ...
                            <div className="text-xs text-gray-600">carbs</div>
                            {mealTargets && (
                              <div className="text-xs text-gray-500">/ {mealTargets.carbs}g</div>
                            )}
                          </div>
                          <div className="text-center">
                            ...
                            <div className="text-xs text-gray-600">fat</div>
                            {mealTargets && (
                              <div className="text-xs text-gray-500">/ {mealTargets.fat}g</div>
                            )}
                          </div>
                        </div>
                            ...
                            <table className="w-full text-sm border-collapse">
                              ...
                              <tbody>
                                {meal.ingredients?.map((ingredient, ingIndex) => {
                                  const cals = ingredient.calories ?? (ingredient as any).kcal ?? (ingredient as any).cal ?? (ingredient as any).energy_kcal ?? 0;
                                  const prot = ingredient.protein ?? (ingredient as any).prot ?? (ingredient as any).proteins ?? (ingredient as any).protein_g ?? 0;
                                  const carbs = ingredient.carbs ?? (ingredient as any).carb ?? (ingredient as any).carbohydrates ?? (ingredient as any).carbohydrates_g ?? 0;
                                  const fat = ingredient.fat ?? (ingredient as any).fats ?? (ingredient as any).fat_g ?? 0;
                                  const amount = ingredient.amount ?? ingredient.quantity ?? "-";
                                  return (
                                    <tr key={ingIndex} className="border-b last:border-0">
                                      <td className="py-2 pr-2 font-medium">{ingredient.name}</td>
