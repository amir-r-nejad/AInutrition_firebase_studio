...
export async function generateStructuredContent<T>(
  ...
  try {
    ...
    if (error.status === 403) {
      throw new Error('API access forbidden. Please check your API key permissions and billing setup.');
    } else if (error.status === 429) {
      throw new Error('API rate limit exceeded. Please try again later.');
    } else if (error.status === 400) {
      throw new Error('Invalid request. Please check your input parameters.');
    } else if (error.status === 401) {
      throw new Error('API key is invalid or expired. Please check your GEMINI_API_KEY.');
