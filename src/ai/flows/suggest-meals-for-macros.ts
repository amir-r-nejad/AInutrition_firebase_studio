...
async function generateWithRAG(
  ...
  try {
    ...
    const allergiesString =
      Array.isArray(input.allergies) && input.allergies.length > 0
        ? input.allergies.join(", ")
        : "";

    const medicalConditionsString =
      Array.isArray(input.medical_conditions) &&
      input.medical_conditions.length > 0
    ...
    const ragPayload = {
      user_profile: {
        ...
        preferred_cuisines: Array.isArray(input.preferred_cuisines)
          ? input.preferred_cuisines.join(", ")
          : input.preferred_cuisines || "",
        dispreferrred_cuisines: Array.isArray(input.dispreferrred_cuisines)
          ? input.dispreferrred_cuisines.join(", ")
          : input.dispreferrred_cuisines || "",
        preferred_ingredients: Array.isArray(input.preferred_ingredients)
          ? input.preferred_ingredients.join(", ")
          : input.preferred_ingredients || "",
        dispreferrred_ingredients: Array.isArray(
    ...
    console.log(
      "Sending request to RAG system:",
      JSON.stringify(ragPayload, null, 2),
    );
        ...
        suggestions: [
          {
            ...
            instructions:
              data.instructions ||
              "Mix ingredients together. Season to taste. Serve immediately.",
      ...
      suggestions: [
        {
          ...
          instructions:
            "Prepare ingredients according to your preference. Season and serve.",
          ingredients: [],
          totalCalories: 0,
          totalProtein: 0,
          totalCarbs: 0,
...
function buildPrompt(input: SuggestMealsForMacrosInput): string {
  ...
  if (
    input.preferred_cuisines &&
    Array.isArray(input.preferred_cuisines) &&
    input.preferred_cuisines.length > 0
  ) {
    preferences.push(
      `Preferred cuisines: ${input.preferred_cuisines.join(", ")}`,
    );
  }
  ...
  if (
    ...
    input.dispreferrred_cuisines.length > 0
  ) {
    preferences.push(
      `Avoid cuisines: ${input.dispreferrred_cuisines.join(", ")}`,
    );
  }
  ...
  if (
    ...
    input.preferred_ingredients.length > 0
  ) {
    preferences.push(
      `Preferred ingredients: ${input.preferred_ingredients.join(", ")}`,
    );
  }
  ...
  if (
    ...
    input.dispreferrred_ingredients.length > 0
  ) {
    preferences.push(
      `Avoid ingredients: ${input.dispreferrred_ingredients.join(", ")}`,
    );
  }
