...
import { Dumbbell, Heart, Target, Clock, MapPin, Utensils, TrendingUp, Activity, PlayCircle, ChevronDown, ChevronUp, Zap, Timer, Repeat, CheckCircle, Youtube, ArrowRight, Calendar, User, BarChart3 } from 'lucide-react';
      ...
      if (response.ok) {
        ...
        if (result.success && result.data) {
          ...
          form.reset({
            ...
            preferred_difficulty_level: savedData.preferred_difficulty_level || undefined,
          ...
          <div className="mt-12 space-y-8">
            ...
            <div className="text-center space-y-4 bg-gradient-to-r from-green-600 to-blue-600 text-white py-12 px-8 rounded-2xl shadow-2xl">
              <div className="flex items-center justify-center gap-3 mb-4">
                ...
                <h2 className="text-4xl font-bold">Your Personalized Exercise Plan</h2>
              </div>
              <p className="text-xl text-white/90 max-w-3xl mx-auto">
                AI-generated workout plan based on your preferences and goals - designed specifically for your fitness journey
              </p>
            ...
            {generatedPlan.error ? (
              ...
              <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      ...
                      <div className="text-center space-y-2">
                        <div className="text-3xl font-bold text-green-600">
                          {Object.values(generatedPlan.weeklyPlan).reduce((total: number, day: any) => total + (day.duration || 0), 0)}
                        </div>
                        <p className="text-green-700 font-medium">Total Minutes</p>
                      </div>
                      <div className="text-center space-y-2">
                        <div className="text-3xl font-bold text-purple-600">
                          {Math.round(Object.values(generatedPlan.weeklyPlan).reduce((total: number, day: any) => total + (day.duration || 0), 0) / Object.keys(generatedPlan.weeklyPlan).length)}
                        </div>
                ...
                <div className="space-y-6">
                  {Object.entries(generatedPlan.weeklyPlan).map(([dayKey, dayPlan]: [string, any], dayIndex) => (
                    <Card key={dayKey} className="border-gray-200 shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden">
                      <CardHeader 
                          ...
                          {dayPlan.mainWorkout && (
                            <div className="space-y-6">
                              ...
                              {dayPlan.mainWorkout.map((exercise: any, idx: number) => {
                                ...
                                return (
                                  <Card key={idx} className="border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50 shadow-lg">
                                      ...
                                      <div className="space-y-4">
                                        ...
                                        <div className="flex items-center justify-between">
                                          <div className="flex items-center gap-3">
                                            ...
                                            <h5 className="text-xl font-bold text-blue-800">{exercise.exerciseName}</h5>
                                          </div>
                                          <div className="flex gap-2">
                                            {exercise.youtubeSearchTerm && (
                                              <Button
                                                ...
                                                onClick={() => window.open(`https://www.youtube.com/results?search_query=${encodeURIComponent(exercise.youtubeSearchTerm)}`, '_blank')}
                                              >
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                          ...
                                          <div className="bg-white/60 backdrop-blur-sm rounded-lg p-3 text-center">
                                            <div className="text-lg font-bold text-orange-600">{exercise.targetMuscles?.length || 0}</div>
                                        ...
                                        {exercise.targetMuscles && exercise.targetMuscles.length > 0 && (
                                          <div className="space-y-2">
                                            <p className="font-medium text-blue-700">Target Muscles:</p>
                                            <div className="flex flex-wrap gap-2">
                                              {exercise.targetMuscles.map((muscle: string, muscleIdx: number) => (
                                                <Badge key={muscleIdx} variant="secondary" className="bg-blue-100 text-blue-700">
                                                  {muscle}
                                                </Badge>
                                              ))}
                                            </div>
                                        ...
                                        {exercise.alternatives && exercise.alternatives.length > 0 && (
                                          <div className="space-y-3">
                                            ...
                                            >
                                              ...
                                              {expandedExercises[exerciseKey] ? <ChevronUp className="w-4 h-4 ml-2" /> : <ChevronDown className="w-4 h-4 ml-2" />}
                                            </Button>

                                            {expandedExercises[exerciseKey] && (
                                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                                                {exercise.alternatives.map((alt: any, altIdx: number) => (
                                                  <Card key={altIdx} className="border-indigo-200 bg-indigo-50">
                                                    <CardContent className="p-4">
                                                      <div className="space-y-3">
                                                        <div className="flex items-center justify-between">
                                                          <h6 className="font-semibold text-indigo-800">{alt.name}</h6>
                                                          {alt.youtubeSearchTerm && (
                                                            <Button
                                                              variant="ghost"
                                                              size="sm"
                                                              className="text-red-600 hover:bg-red-50 p-1"
                                                              onClick={() => window.open(`https://www.youtube.com/results?search_query=${encodeURIComponent(alt.youtubeSearchTerm)}`, '_blank')}
                                                            >
                                                              <Youtube className="w-4 h-4" />
                                                            </Button>
                                                          )}
                                                        </div>
                                                        <p className="text-sm text-indigo-700">{alt.instructions}</p>
                                                      </div>
                                                    </CardContent>
                                                  </Card>
                                                ))}
