...
// --- Macro Splitter Page ---
export async function getMacroSplitterData(userId: string): Promise<{
  dailyTargets: {
    calories: number;
    protein_g: number;
    carbs_g: number;
  ...
  if (!profile) {
    return {
      ...
      sourceMessage: 'Could not find user profile.',
    };
  }
  ...
  const mealDistributions =
    profile.mealDistributions &&
    Array.isArray(profile.mealDistributions) &&
    profile.mealDistributions.length > 0
      ? (profile.mealDistributions as MealMacroDistribution[])
      : null;

  return { dailyTargets: targets, mealDistributions, sourceMessage };
}

export async function saveMealDistributions(
  userId: string,
  distributions: MealMacroDistribution[]
) {
  if (!userId) throw new Error('User ID is required.');
  try {
    const userProfileRef = doc(db, 'users', userId);
    const distributionsToSave = preprocessDataForFirestore(distributions);
    await setDoc(
      userProfileRef,
    ...
    throw new Error('Could not save meal distributions.');
  }
}

export async function resetMealDistributions(userId: string) {
  if (!userId) throw new Error('User ID is required.');
  try {
    const userProfileRef = doc(db, 'users', userId);
    ...
    throw new Error('Could not reset meal distributions.');
  }
}
