...
export async function POST(request: Request) {
  try {
    ...
    if (
      !weeklyPlan ||
      !weeklyPlan[dayIndex] ||
      !weeklyPlan[dayIndex].meals ||
      !weeklyPlan[dayIndex].meals[mealIndex]
    ) {
      console.error("❌ API: Invalid meal plan structure", {
        ...
        mealExists: weeklyPlan?.[dayIndex]?.meals?.[mealIndex]
          ? "exists"
          : "missing",
      });
      return json(
        { error: "Invalid meal plan structure or meal not found" },
        400,
      );
    }
    ...
    console.log(
      "🔧 API: Optimizing meal:",
      JSON.stringify(mealToOptimize, null, 2),
    );
    ...
    if (ingredients.length === 0) {
      ...
      return json(
        { error: "No valid ingredients found for optimization" },
        400,
      );
    }

    console.log(
      "🧮 API: Converted ingredients:",
    ...
    const within5 = (value: number, target: number) =>
      Math.abs(value - target) <= 0.05 * Math.max(50, target);
    ...
    if (needsAugment) {
      console.warn(
        "⚠️ API: Augmenting ingredients to improve feasibility/accuracy",
      );
      ...
      const canonical = (s: string) => {
        ...
        if (n.includes("cottage") && n.includes("cheese"))
        ...
        if (n.includes("peanut") && n.includes("butter"))
      ...
      const existingCanonical = new Set(
        workingIngredients.map((i) => canonical(i.name)),
      );
      ...
      const achieved = optimizationResult.achieved || {
        calories: 0,
        protein: 0,
        carbs: 0,
        fat: 0,
      ...
      if (deficits.prot > 0) {
        if (!existingCanonical.has(canonical("Whey Protein Isolate"))) {
          ...
          !existingCanonical.has(canonical("Greek Yogurt (Non-Fat)"))
        ) {
          workingIngredients.push(helpers[0]);
          existingCanonical.add(canonical("Greek Yogurt (Non-Fat)"));
        } else if (
          !existingCanonical.has(canonical("Cottage Cheese (Low-Fat)"))
        ) {
      ...
      if (
        deficits.cal > 0 &&
        workingIngredients.length === ingredients.length &&
      ...
      if (stillOutside) {
        ...
        if (protLow && (carbsHigh || fatHigh)) {
          if (
            !existingCanonical.has(canonical("Whey Protein Isolate")) &&
            !workingIngredients.find(
              (i) => canonical(i.name) === canonical("Whey Protein Isolate"),
            )
          ) {
            workingIngredients.push(helpers[2]);
            existingCanonical.add(canonical("Whey Protein Isolate"));
          } else if (
            !existingCanonical.has(canonical("Egg Whites")) &&
            !workingIngredients.find(
              (i) => canonical(i.name) === canonical("Egg Whites"),
            )
          ) {
            workingIngredients.push(helpers[3]);
            existingCanonical.add(canonical("Egg Whites"));
      ...
      if (!optimizationResult.feasible) {
        ...
        return json(
          {
            error:
              "Optimization not feasible even after adding helper ingredients",
          },
          400,
        );
      }
    ...
    console.log(
      "🍽️ API: Optimized meal:",
      JSON.stringify(optimizedMeal, null, 2),
    );
    ...
    const weeklySummary: {
      ...
      (
        ...
        totalCalories:
          summary.totalCalories + (day.daily_totals?.calories || 0),
