...
// Enhanced fitness function with adaptive weights and constraint relaxation
function calculateFitnessAdvanced(genes: number[], ingredients: Ingredient[], targets: Targets, iteration: number = 0): number {
  let totalCal = 0, totalProt = 0, totalCarb = 0, totalFat = 0;
  ...
  // Adaptive weights based on iteration (relax constraints over time)
  const progress = Math.min(iteration / 500, 1); // Normalize progress
  const baseWeight = 1.0;
  const adaptiveWeight = baseWeight + progress * 2; // Increase tolerance over time

  // Calculate deviations
  const calDeviation = Math.abs(totalCal - targets.calories);
  const protDeviation = Math.abs(totalProt - targets.protein);
  const carbDeviation = Math.abs(totalCarb - targets.carbs);
  const fatDeviation = Math.abs(totalFat - targets.fat);

  // Adaptive tolerance based on progress
  const tolerance = 0.1 + progress * 0.2; // Start with 10%, increase to 30%
  
  // Check if within adaptive tolerance
  const calWithinTolerance = calDeviation / targets.calories <= tolerance;
  const protWithinTolerance = protDeviation / targets.protein <= tolerance;
  const carbWithinTolerance = carbDeviation / targets.carbs <= tolerance;
  const fatWithinTolerance = fatDeviation / targets.fat <= tolerance;

  // Weighted deviations with adaptive weights
  const calWeight = calWithinTolerance ? 0.5 : 2.0; // Reduce weight if within tolerance
  const protWeight = protWithinTolerance ? 1.0 : 4.0;
  const carbWeight = carbWithinTolerance ? 0.8 : 2.0;
  const fatWeight = fatWithinTolerance ? 1.5 : 5.0; // Higher weight for fat accuracy

  const weightedDeviation = 
    calDeviation * calWeight +
    protDeviation * protWeight +
    carbDeviation * carbWeight +
    fatDeviation * fatWeight;

  // Bonus for being close to targets
  let closenessBonus = 0;
  if (calWithinTolerance && protWithinTolerance && carbWithinTolerance && fatWithinTolerance) {
    const totalDeviation = calDeviation + protDeviation + carbDeviation + fatDeviation;
    closenessBonus = -totalDeviation * 0.1; // Negative because lower fitness is better
  }

  // Penalty for excessive amounts
  const amountPenalty = genes.reduce((sum, amount) => {
    if (amount > 400) return sum + (amount - 400) * 0.05;
    return sum;
  }, 0);

  return weightedDeviation + amountPenalty + closenessBonus;
}
...
function createIndividual(ingredientsCount: number): Individual {
  const genes = Array.from({ length: ingredientsCount }, () => 
    Math.random() * ALGORITHM_CONFIG.ga.maxAmount
  );
...
function selectParent(population: Individual[]): Individual {
  ...
  for (let i = 0; i < ALGORITHM_CONFIG.ga.tournamentSize; i++) {
...
function crossover(parent1: Individual, parent2: Individual): Individual[] {
  if (Math.random() > ALGORITHM_CONFIG.ga.crossoverRate) {
...
function mutate(individual: Individual): void {
  for (let i = 0; i < individual.genes.length; i++) {
    if (Math.random() < ALGORITHM_CONFIG.ga.mutationRate) {
      const stepSize = ALGORITHM_CONFIG.ga.maxAmount * 0.1;
      const mutation = (Math.random() - 0.5) * 2 * stepSize;
      
      individual.genes[i] += mutation;
      individual.genes[i] = Math.max(0, Math.min(ALGORITHM_CONFIG.ga.maxAmount, individual.genes[i]));
    }
  }
}
...
function optimizeWithGeneticAlgorithm(
  ...
  console.log("📊 Population:", ALGORITHM_CONFIG.ga.populationSize, "Generations:", ALGORITHM_CONFIG.ga.generations);

  let population: Individual[] = Array.from({ length: ALGORITHM_CONFIG.ga.populationSize }, 
  ...
  for (let generation = 0; generation < ALGORITHM_CONFIG.ga.generations; generation++) {
    ...
    if (generationsWithoutImprovement > 150) {
    ...
    for (let i = 0; i < ALGORITHM_CONFIG.ga.eliteSize; i++) {
      newPopulation.push({ ...population[i] });
    }

    while (newPopulation.length < ALGORITHM_CONFIG.ga.populationSize) {
      ...
      if (newPopulation.length < ALGORITHM_CONFIG.ga.populationSize) {
  ...
  if (bestIndividual) {
    ...
    ingredients.forEach((ingredient, index) => {
      if (bestIndividual!.genes[index] > 0.5) {
...
// ===== PARTICLE SWARM OPTIMIZATION =====
function createParticle(ingredientsCount: number): Particle {
  const position = Array.from({ length: ingredientsCount }, () => 
    Math.random() * ALGORITHM_CONFIG.pso.maxAmount
  );
  
  const velocity = Array.from({ length: ingredientsCount }, () => 
    (Math.random() - 0.5) * ALGORITHM_CONFIG.pso.maxAmount * 0.1
  );
...
function optimizeWithPSO(
  ingredients: Ingredient[],
  targets: Targets
): OptimizationResult {
  console.log("🌊 Starting Particle Swarm Optimization");
  console.log("📊 Particles:", ALGORITHM_CONFIG.pso.particles, "Iterations:", ALGORITHM_CONFIG.pso.iterations);

  const particles: Particle[] = Array.from({ length: ALGORITHM_CONFIG.pso.particles }, 
    () => createParticle(ingredients.length)
  );
  ...
  particles.forEach(particle => {
    particle.fitness = calculateFitness(particle.position, ingredients, targets);
  ...
  for (let iteration = 0; iteration < ALGORITHM_CONFIG.pso.iterations; iteration++) {
    particles.forEach(particle => {
      // Update velocity
      for (let i = 0; i < particle.position.length; i++) {
        ...
        particle.velocity[i] = ALGORITHM_CONFIG.pso.w * particle.velocity[i] +
          ALGORITHM_CONFIG.pso.c1 * r1 * (particle.bestPosition[i] - particle.position[i]) +
          ALGORITHM_CONFIG.pso.c2 * r2 * (globalBestPosition[i] - particle.position[i]);
      }

      // Update position
      for (let i = 0; i < particle.position.length; i++) {
        particle.position[i] += particle.velocity[i];
        particle.position[i] = Math.max(0, Math.min(ALGORITHM_CONFIG.pso.maxAmount, particle.position[i]));
      }
    ...
    if (iterationsWithoutImprovement > 100) {
      console.log(`🔄 PSO Early stopping at iteration ${iteration}`);
      break;
    }

    if (iteration % 100 === 0) {
      const currentBest = calculateMacros(globalBestPosition, ingredients);
      console.log(`🌊 Iter ${iteration}: Fitness=${globalBestFitness.toFixed(2)}, ` +
                  `Macros: ${currentBest.calories}cal/${currentBest.protein}p/${currentBest.carbs}c/${currentBest.fat}f`);
    }
  }

  const achieved = calculateMacros(globalBestPosition, ingredients);
  
  const ingredientAmounts: { [key: string]: number } = {};
  ingredients.forEach((ingredient, index) => {
    if (globalBestPosition[index] > 0.5) {
      ingredientAmounts[ingredient.name] = Math.round(globalBestPosition[index] * 100) / 100;
    }
  });

  console.log("🎯 PSO Optimization Complete!");
...
function optimizeWithSimulatedAnnealing(
  ...
  console.log("📊 Iterations:", ALGORITHM_CONFIG.sa.iterations);

  let currentSolution = Array.from({ length: ingredients.length }, () => 
    Math.random() * ALGORITHM_CONFIG.sa.maxAmount
  );
  
  let currentFitness = calculateFitness(currentSolution, ingredients, targets);
  let bestSolution = [...currentSolution];
  let bestFitness = currentFitness;
  
  let temperature = ALGORITHM_CONFIG.sa.initialTemp;
  let iterationsWithoutImprovement = 0;

  for (let iteration = 0; iteration < ALGORITHM_CONFIG.sa.iterations; iteration++) {
    // Generate neighbor solution
    const neighborSolution = [...currentSolution];
    const randomIndex = Math.floor(Math.random() * neighborSolution.length);
    const stepSize = ALGORITHM_CONFIG.sa.maxAmount * 0.1;
    const change = (Math.random() - 0.5) * 2 * stepSize;
    
    neighborSolution[randomIndex] += change;
    neighborSolution[randomIndex] = Math.max(0, Math.min(ALGORITHM_CONFIG.sa.maxAmount, neighborSolution[randomIndex]));
    
    const neighborFitness = calculateFitness(neighborSolution, ingredients, targets);
    ...
    if (iterationsWithoutImprovement > 200) {
      console.log(`🔄 SA Early stopping at iteration ${iteration}`);
      break;
    }
    
    // Cool down
    temperature *= ALGORITHM_CONFIG.sa.coolingRate;
    
    if (iteration % 200 === 0) {
      const currentBest = calculateMacros(bestSolution, ingredients);
      console.log(`🔥 Iter ${iteration}: Fitness=${bestFitness.toFixed(2)}, ` +
                  `Macros: ${currentBest.calories}cal/${currentBest.protein}p/${currentBest.carbs}c/${currentBest.fat}f`);
    }
  }

  const achieved = calculateMacros(bestSolution, ingredients);
  
  const ingredientAmounts: { [key: string]: number } = {};
  ingredients.forEach((ingredient, index) => {
    if (bestSolution[index] > 0.5) {
      ingredientAmounts[ingredient.name] = Math.round(bestSolution[index] * 100) / 100;
    }
  });

  console.log("🎯 SA Optimization Complete!");
  console.log("📊 Best fitness:", bestFitness.toFixed(2));
  console.log("🥗 Final macros:", achieved);
...
// ===== HYBRID OPTIMIZATION =====
function optimizeWithHybridAlgorithm(
  ingredients: Ingredient[],
  targets: Targets
): OptimizationResult {
  console.log("🚀 Starting Hybrid Optimization (GA + PSO + SA)");
  
  // Run all three algorithms
  const results = [
    optimizeWithGeneticAlgorithm(ingredients, targets),
    optimizeWithPSO(ingredients, targets),
    optimizeWithSimulatedAnnealing(ingredients, targets)
  ];
  
  // Find the best result
  let bestResult = results[0];
  for (let i = 1; i < results.length; i++) {
    if (results[i].result < bestResult.result) {
      bestResult = results[i];
    }
  }
  
  console.log("🎯 Hybrid Optimization Complete!");
  console.log("🏆 Best algorithm:", bestResult.algorithm_used);
  console.log("📊 Best fitness:", bestResult.result.toFixed(2));
  
  return {
    ...bestResult,
    algorithm_used: `Hybrid (${bestResult.algorithm_used})`
  };
}

// Enhanced PSO with adaptive parameters
function optimizeWithPSOEnhanced(
  ingredients: Ingredient[],
  targets: Targets
): OptimizationResult {
  console.log("🌊 Starting Enhanced Particle Swarm Optimization");
  console.log("📊 Particles:", ALGORITHM_CONFIG.pso.particles, "Iterations:", ALGORITHM_CONFIG.pso.iterations);
  ...
  // Initialize particles
  particles.forEach(particle => {
    particle.fitness = calculateFitnessAdvanced(particle.position, ingredients, targets, 0);
    particle.bestFitness = particle.fitness;
    particle.bestPosition = [...particle.position];
  ...
  for (let iteration = 0; iteration < ALGORITHM_CONFIG.pso.iterations; iteration++) {
    // Adaptive parameters
    const w = ALGORITHM_CONFIG.pso.w * (1 - iteration / ALGORITHM_CONFIG.pso.iterations * 0.3);
    const c1 = ALGORITHM_CONFIG.pso.c1 * (1 + iteration / ALGORITHM_CONFIG.pso.iterations * 0.2);
    const c2 = ALGORITHM_CONFIG.pso.c2 * (1 + iteration / ALGORITHM_CONFIG.pso.iterations * 0.2);
    ...
    particles.forEach(particle => {
      ...
      // Evaluate with adaptive fitness
      particle.fitness = calculateFitnessAdvanced(particle.position, ingredients, targets, iteration);
    ...
    if (iterationsWithoutImprovement > 150) {
      console.log(`🔄 Enhanced PSO Early stopping at iteration ${iteration}`);
      break;
    }
  ...
  console.log("🎯 Enhanced PSO Optimization Complete!");
  console.log("📊 Best fitness:", globalBestFitness.toFixed(2));
  console.log("🥗 Final macros:", achieved);
  ...
  const toleranceCheck = {
    calories: Math.abs(achieved.calories - targets.calories) / targets.calories <= tolerance,
    protein: Math.abs(achieved.protein - targets.protein) / targets.protein <= tolerance,
    carbs: Math.abs(achieved.carbs - targets.carbs) / targets.carbs <= tolerance,
    fat: Math.abs(achieved.fat - targets.fat) / targets.fat <= tolerance
  };
  ...
  return {
    ...
    ingredients: ingredientAmounts,
    achieved,
    algorithm_used: "Enhanced Particle Swarm Optimization",
    error: isAcceptable ? undefined : "Could not achieve targets within 15% tolerance"
  };
}
...
function generateContextualHelpers(
  ...
  for (const category of helperCategories.slice(0, 6)) {
...
// Main optimization function with enhanced algorithms
export function optimizeMealAdvanced(
  ingredients: Ingredient[],
  targets: Targets,
  mealName: string = 'meal',
  algorithm: 'ga' | 'pso' | 'sa' | 'hybrid' | 'enhanced' = 'enhanced'
): OptimizationResult {
  console.log("🧬 Starting Advanced Multi-Algorithm Optimization");
  ...
  try {
    ...
    switch (algorithm) {
      case 'ga':
        result = optimizeWithGeneticAlgorithm(ingredients, targets);
        break;
      case 'pso':
        result = optimizeWithPSO(ingredients, targets);
        break;
      case 'sa':
        result = optimizeWithSimulatedAnnealing(ingredients, targets);
        break;
      case 'enhanced':
        result = optimizeWithPSOEnhanced(ingredients, targets);
        break;
      case 'hybrid':
      default:
        result = optimizeWithHybridAlgorithm(ingredients, targets);
        break;
    }

    // Check if result is within acceptable tolerance (15% for enhanced)
    const tolerance = algorithm === 'enhanced' ? 0.15 : 0.05;
    const isWithinTolerance = result.feasible && result.achieved && 
      Math.abs(result.achieved.calories - targets.calories) / targets.calories <= tolerance &&
      Math.abs(result.achieved.protein - targets.protein) / targets.protein <= tolerance &&
      Math.abs(result.achieved.carbs - targets.carbs) / targets.carbs <= tolerance &&
      Math.abs(result.achieved.fat - targets.fat) / targets.fat <= tolerance;

    if (isWithinTolerance) {
      console.log(`✅ Original ingredients achieved targets within ${tolerance * 100}% tolerance`);
      return result;
    }
    ...
    // Try optimization with helpers using the same algorithm
    const enhancedResult = (() => {
      switch (algorithm) {
        case 'ga':
          return optimizeWithGeneticAlgorithm(enhancedIngredients, targets);
        case 'pso':
          return optimizeWithPSO(enhancedIngredients, targets);
        case 'sa':
          return optimizeWithSimulatedAnnealing(enhancedIngredients, targets);
        case 'enhanced':
          return optimizeWithPSOEnhanced(enhancedIngredients, targets);
        case 'hybrid':
        default:
          return optimizeWithHybridAlgorithm(enhancedIngredients, targets);
      }
    })();
    ...
    if (enhancedResult.feasible) {
      ...
      console.log("✅ Enhanced optimization with helpers completed");
...
export function convertOptimizationToMeal(
  ...
  // Create description mentioning the algorithm used
  let description = `Optimized ${originalMeal.mealTitle} using ${optimizationResult.algorithm_used || 'advanced optimization algorithm'} to achieve precise macro targets with intelligent ingredient selection.`;
